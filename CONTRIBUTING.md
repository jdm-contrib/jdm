...
- `difficulty`: This is an indicator used on the site to determine the difficulty of account deletion. Use one of:
  ...
  - `only_required`: Sites that only allow you to delete your account if you live in an area with privacy rights or where otherwise required to by a policy (e.g the iOS App Store). These are only used for websites that require proof that you are covered by local law or policy and verify it..
