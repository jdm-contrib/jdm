...
- `difficulty`: This is an indicator used on the site to determine the difficulty of account deletion. Use one of:
  ...
  - `only_required`: Sites that only allow you to delete your account if you live in an area with privacy rights or where otherwise required to by policy (e.g the iOS App Store). This designation is meant for sites that verify you are covered by local law or policy, such as by requiring proof of residency or only allowing deletion from an iOS app, not including sites that only link to a form and do not further verify eligibility.
