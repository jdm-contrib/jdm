...
- `notes`: *(optional)* Notes will be shown when someone expands the info on that service. They should start as uppercase and include additional information you might need to delete your account (e.g. Revolut) or consequences of deleting your account (e.g. iTunes). 
  - If you are drastically changing existing notes, also delete its translations, unless you have proficiency in one or more of them.
  - If you want to include links in the notes, you should use markdown syntax, e.g.: `Visit [this page](https://example.com).` would become "Visit [this page](https://example.com)."
  - When an `email` is set or when the difficulty is `easy` or `hard`, we have default notes. Check the translation JSON file and leave it out if you were going to state the same thing.
...
- `domains`: This is used by the [Unofficial Chrome extension](https://github.com/fregante/jdm) and [Firefox Add-on](https://github.com/jdm-contrib/justdelete.me-firefox-addon) and is a list a list of domains valid for that service without any leading 'https://' or 'www'.
...
8. Name the commit "Add SERVICE as DIFFICULTY" if adding a new one or "Update FIELD for SERVICE" if updating.
